...
| **v0.2 (Next Baseline)** | **Phase 3** | Zenoh P2P heartbeats, dynamic 15.05s stale worker eviction under WAN drops, **+ Early CI/CD Agent Code Auditors**. | Asynchronous P2P telemetry mesh, AEAD encrypted telemetry envelopes, automated lint/type check agents. |
| **v0.3 (Accelerated)** | **Phase 3.5 / 4** | **+ Web/Desktop Visual Dashboard (One-click host & prompt playground)**, Sandboxed Docker runtimes, P2P Model Parallelism across consumer GPUs. | Visual Control Plane (WebUI/Desktop App), container isolation, P2P tensor layer sharding & sequential streaming. |
