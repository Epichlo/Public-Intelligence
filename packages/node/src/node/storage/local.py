"""Concrete local disk storage driver for Public Intelligence artifacts."""
...
from .base import ArtifactMetadata, ArtifactStore
...
DEFAULT_ARTIFACT_DIR = os.path.join(tempfile.gettempdir(), "public_intelligence", "artifacts")
...
class LocalDiskArtifactStore(ArtifactStore):
    ...
    async def save_artifact(
        ...
        # Write raw binary data to {artifact_id}.bin inside the base directory
    ...
    async def get_artifact(self, uri: str) -> bytes:
        """Retrieve the raw binary data of an artifact using its URI.
