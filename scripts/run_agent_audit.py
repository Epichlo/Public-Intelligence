"""Autonomous multi-agent CI/CD code auditor runner for Node."""
...
async def run_verifier_task(
    ...
    try:
        # Run ruff check
        subprocess.run(
            [ruff_cmd, "check", "."], check=True, capture_output=True, text=True
        )
        ...
        # Run pytest
        subprocess.run([pytest_cmd], check=True, capture_output=True, text=True)
