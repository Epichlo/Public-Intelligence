...
**The 7 Zenoh tests that fail in a sandbox are an environment limit, not a defect.**
They bind `tcp/[::]:0` and the container has no IPv6 (`EAFNOSUPPORT`). They pass
where IPv6 exists. Do not "fix" them by weakening the assertions.
