...
| 2.5 | **Node eviction is trustworthy.** **This line asserted something untrue and is corrected:** it said "deathrattle and stale-eviction now apply correctly", but *stale-eviction did not exist* — no periodic sweep, no `last_seen` check on nodes anywhere in the Scheduler. A host left the registry only by unregistering gracefully or by an unauthenticated deathrattle. 2.7 built the sweep, because it had to remove the deathrattle's write. What remains of 2.5 is the original observation: `_process_deathrattle` logged success unconditionally, and eviction should be *observable* rather than silent. | **partial** | 2.1 |
...
| 2.9 | **Lint the cross-package tests, and catch implicit text encodings.** Two gaps found when 2.7 passed the local gate and then failed CI on Windows. (a) `scripts/verify.sh` runs `ruff check ./packages` and never touches the root `tests/` directory, so the cross-package suite — the wire contract and every ratchet — is unlinted; the full ruleset surfaces 22 pre-existing errors there, and there is no ruff config at the repo root, which is also why a stray `ruff format ./tests` reformats at the wrong line length. (b) A `Path.read_text()` with no encoding reads as cp1252 on Windows and UTF-8 everywhere else, so a single non-ASCII byte in any scanned file fails only on Windows. Ruff's `PLW1514` catches the class but needs preview mode enabled, which pulls in other unstable rules. | **not started** | — |
