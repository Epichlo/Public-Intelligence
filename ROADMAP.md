...
| W8 | **Every node either installer produced was called `node-local`, so two of them collided.** `env_prefix = "NODE_"` resolves the field `node_id` to `NODE_NODE_ID`, and both installers wrote `NODE_ID` — which bound to nothing. pydantic-settings cannot warn about that: an unmatched variable is indistinguishable from an unrelated one, so a typo'd setting is not an error, it is a default. Invisible with one node; with two it is a collision on the registry key *and* on the mesh queryable `public-intelligence/net/{node_id}/infer`. Fixed with `AliasChoices("NODE_ID", "NODE_NODE_ID")` so already-installed nodes start working without anyone editing a file. | **done** |
| W9 | **The ratchet that was supposed to catch W8 could not have.** `test_compose_env_matches_settings.py` says in its own docstring that it "would have caught the original bug" of `NODE_ID` where `NODE_NODE_ID` was meant. Its helper added the bare field name as an accepted alias — which for the field `node_id` is literally `NODE_ID` — so it blessed the broken variable. The compose file was fixed by hand and the test took the credit. The helper now adds the bare name only when the model sets no `env_prefix`, lives in one place (`tests/env_binding.py`) rather than two copies, and is ratcheted over the installers as well as the compose file. Worth recording: with the old helper restored **and** W8 reintroduced, four of the five new tests still pass — only the one that constructs two `Settings` objects and compares them catches it. Reading files proves less than exercising behaviour. | **done** |
| W10 | **Hosting a node took three steps you had to know in order** — clone, `install.sh`, then `launch_host_node.sh start` — and there was no `curl \| bash` on-ramp, which is friction on exactly the path the two-machine test found painful ("six manual workarounds"). Now one line: `scripts/bootstrap.sh` clones or updates the repo and runs `install.sh --start`, forwarding its flags, so the command ends with a *running* node. `--start` is opt-in and launches only after a successful install — a plain `./install.sh` is unchanged, and a failed install never leaves a "started" dead daemon (the D-3 lesson). `scripts/verify_install.sh` proves both for real: a node answering `/health`, and the one-liner cloning and forwarding flags hermetically against a local origin. See `specs/single-command-node-hosting.md`. | **done** |
| W7 | **One header carried two meanings, so every node's "per-node" credential was the fleet secret.** `register_node` stored `X-Network-Auth-Token` as the node's own credential while `verify_auth_token` had already required that same header to equal the fleet secret — so the key 2.7 seals mesh envelopes with could only ever *be* the shared one, and 2.7's central claim ("closing the insider case a fleet-wide secret never could") was false in the only configuration anyone deploys. Split by [D9](docs/decisions/D9-admission-is-not-identity.md) into `X-Network-Auth-Token` (compared, never stored) and `X-Node-Credential` (stored, never compared). A node sending no credential still registers, so no running host is stranded — which means **the weakness persists for un-upgraded nodes by design**, not by omission. W5's fix also had to be undone: it assigned the operator's fleet token to the variable holding the per-install credential, in both installers and thence into the dashboard's env. | **done** |
...
is not something a container fails to provide — it is zenoh's **default** listener,
chosen because no test named one. They now name `tcp/127.0.0.1:0` explicitly and run
anywhere, IPv6 or not. No assertion changed.

Worth keeping as a warning rather than deleting: that explanation survived two
commits because "the sandbox is missing something" is comfortable and ends the
investigation. One command falsified it.
...
| C6 | **DONE — and this row was stale, which is the thing this file exists not to be.** `packages/website` has 36 vitest tests across five files (4.1), `package.json` carries `test` and `typecheck` scripts, and `scripts/verify.sh` runs both. `vitest.config.mts` sets `passWithNoTests: false`, so the step cannot go green on zero test files. Pinned by `test_the_gate_checks_the_website`. Original finding: **`packages/website` has zero tests** — `package.json` has `lint` and no `test`. | **done** |
| C7 | **DONE — and this row was stale too.** The gate runs `mypy tests` (which only checks anything because both packages ship a PEP 561 `py.typed` marker — without it mypy answers "missing library stubs" for every import and the step verifies nothing), plus `website lint`, `website types` and `website tests`. Pinned by `test_the_gate_type_checks_the_root_tests` and `test_the_gate_checks_the_website`. Original finding: **The gate does not type-check `tests/` and does not touch the website at all.** | **done** |
...
| # | Feature | Status | Depends on |
|---|---------|--------|-----------|
| 4.1 | **DONE.** 36 vitest tests across the three proxy routes, the contribution panel and the SSE parser. The parser was the point: it lived inline in `playground/page.tsx` tangled with React state setters, and it is the one place on the website where being subtly wrong is **invisible** — a dropped delta reads as the model not saying that word. Extracted to `lib/sse.ts` and tested against the cases that produce a plausible-but-wrong transcript: a chunk boundary mid-line, `data:` with no space, a whitespace-only delta dropped as falsy. Fixed while extracting: the old `catch` appended the **raw malformed line** to the transcript, so a bad frame showed `{"choices":[{"delta":` to the user as model output. Earlier: the chat proxy was sending the fleet secret as a Bearer JWT. | **done** | — |
| 4.2 | **DONE — the alerting half is now `scripts/watch_scheduler.py`, and it is a separate process for the reason this line already gave.** A background task inside the Scheduler cannot report that the Scheduler is down, which is the failure an operator most needs to hear about; it would be silent in exactly the case it exists for. The watcher polls `/metrics`, applies rules, prints one JSON object per poll, and exits 0/1/2 so a cron line is a health check. Two things it refuses to do: an **empty window is `idle`, never `ok`** — `/metrics` reports `failure_ratio 0.0` when it holds no requests, because 0/0 renders as 0, so the naive check calls a network that has served nothing flawless — and a **ratio rule fires only after consecutive breaches over a minimum sample**, because a one-poll blip that pages someone is the concrete mechanism by which an alerter becomes noise. A rejected token reports `unauthorised`, not `unreachable`, so a wrong credential does not send someone to restart a healthy service. **Delivery is out of scope**: it prints, and something else decides what that means. See `specs/something-outside-the-process-notices.md`. Original line: **PARTIAL — aggregation done, alerting deliberately not.** `GET /metrics` reports request counts, token totals, per-node distribution and the **failure ratio** an operator actually watches, so something other than CI can notice breakage. JSON rather than Prometheus text format on purpose: an exposition endpoint implies a scrape contract (naming, label cardinality, HELP/TYPE) and getting that subtly wrong is harder to notice than not having it; adopting it later is a serialiser change. **Alerting is not here and is not pretended** — something outside the process has to poll and decide. Every counter is labelled as windowed, because a figure read as all-time that silently resets when the buffer wraps is worse than no figure. | **done** | 2.1 |
