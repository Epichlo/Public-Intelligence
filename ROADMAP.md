...
| 2.4 | **Authenticate `/v1/batch`.** `POST /v1/batch` and `GET /v1/batch/{id}` have no auth dependency at all. **Batch persistence waits on this**, and 2.1 deliberately left `_BATCH_TASKS` in memory: giving an unauthenticated write path a database converts unbounded *memory* growth by an anonymous caller into unbounded *disk* growth. (Separately, `submit_batch_job` fabricates its results and dispatches nothing, so today there is no record of work to make durable.) | **not started** | — |
