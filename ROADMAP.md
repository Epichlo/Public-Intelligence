...
| C2 | **MOSTLY DONE, and it was far more urgent than "tidiness" implied.** Moving this code found **three live defects that 664 passing tests did not**, all caused by cut-feature plumbing wired into live paths: (a) **every streamed completion was republished onto the Zenoh mesh in plaintext**, on a key any peer can `**`-subscribe to, with no subscriber in the codebase — while 2.7 had just spent a protocol change encrypting *telemetry* on that same mesh; (b) **streaming deadlocked after 4 chunks**, because the flow-control window waits for an ACK nothing has ever sent — so streaming was broken on exactly the deployment this project exists for; (c) **every node opened an unauthenticated wildcard subscriber** on `.../tensors/*` that deserialised attacker-controlled bytes and **read and unlinked host shared memory by attacker-supplied name**. `local_boundary`, `transport`, `boundary_engine`, `kv_cache`, `quantization` (both copies each) and 10 test files are now in `experimental/`, **linted but not tested** — excluding them outright would let 2,000 lines rot, and it was the test *count* that needed to mean something. Enforced by `tests/test_experimental_is_quarantined.py`. **Still open: `consensus` (530)**, which is genuinely wired into `zenoh_router`, `node_registry`, `ingress` and `openai` — a real refactor on a live dispatch path, not a file move. | N1 |
...
| 3.4 | **PARTIAL — the API exists, the dashboard does not read it yet.** `GET /nodes/{id}/usage` returns what a host's machine served and what it **contributed**, and `GET /usage` returns the recent fleet-wide tail. The word is *contributed*, never *earned*: D2 cut redemption, so a dashboard saying "earned" would promise something the project has explicitly declined. Both responses label their **window**, because the in-memory tail is bounded and a caller assuming all-time would draw a graph that silently flattens once it wraps. The wiring into `packages/website` is what remains. | **partial** | 3.3 |
