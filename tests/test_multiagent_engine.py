"""Unit tests for multi-agent engine in Node."""
...
async def test_parallel_worker_execution() -> None:
    ...
    tasks = [
        SubTask(task_id=f"t_{i}", role=AgentRole.CODER, description=f"Task {i}")
        for i in range(4)
    ]

    async def custom_handler(
        task: SubTask, _read_state: SharedState
    ) -> SharedStateDelta:
    ...
    results = await orchestrator.run_batch(
        tasks, handlers={AgentRole.CODER.value: custom_handler}
    )
...
async def test_dependency_resolution() -> None:
    ...
    async def handler(task: SubTask, _read_state: SharedState) -> SharedStateDelta:
...
async def test_max_depth_exceeded() -> None:
    ...
    t = SubTask(
        task_id="deep", role=AgentRole.VERIFIER, description="Deep task", depth=4
    )
